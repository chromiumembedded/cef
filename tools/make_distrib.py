...
def validate_options(options, platform):
  ...
  if options.ozone and platform != 'linux':
    return '--ozone is only supported on Linux.'
  symbols_only = (options.symbolsonly, options.debugsymbolsonly,
                  options.releasesymbolsonly)
  if (options.nosymbols and any(symbols_only)) or sum(symbols_only) > 1:
    return 'Invalid combination of build options.'
  return None


def get_architecture(options):
  if options.x64build:
    return ('64', 'x64', '_GN_x64')
  if options.armbuild:
...
def get_distribution_mode(options):
  if options.minimal:
    return 'minimal'
  if options.client:
    return 'client'
  ...
  if options.releasesymbolsonly:
    return 'release-symbols'
  if options.symbolsonly:
    return 'symbols'
  return 'standard'


def get_output_name(cef_version, platform, platform_arch, mode, options):
  output_dir_base = 'cef_binary_' + cef_version
  if options.distribsubdir == '':
    if platform == 'mac':
      # For backwards compatibility keep the old default directory name on
      # macOS.
  ...
  if mode in ('minimal', 'client', 'sandbox', 'tools'):
    output_dir_name += '_' + mode
  if options.ozone:
    output_dir_name += '_ozone'
  return output_dir_base, output_dir_name


def main(argv=None):
  """Build the requested CEF binary distribution."""
  global options
  global platform
  global script_dir, cef_dir, src_dir
  global cef_url, cef_rev, cef_commit_number
  global chromium_url, chromium_rev, date
  global version_formatter, cef_ver, chromium_ver
  global archive_dirs, platform_arch, binary_arch
  global output_dir_base, output_dir_name, mode, output_dir
  global cef_paths, cef_paths2
  global build_dir_debug, build_dir_release
  global include_dir, cmake_dir, libcef_dll_dir
  global tests_dir, shared_dir, cefclient_dir
  global cefsimple_dir, cefsimple_capi_dir, ceftests_dir
  ...
  platform_arch, binary_arch, build_dir_suffix = get_architecture(options)
  mode = get_distribution_mode(options)
  output_dir_base, output_dir_name = get_output_name(
      cef_ver, platform, platform_arch, mode, options)
  ...
  if mode == 'tools':
    ...
    if mode == 'client':
      binaries.append({
          'path':
              'cefsimple.exe' if platform_arch == 'arm64' else 'cefclient.exe'
      })
    else:
      if mode == 'sandbox':
    ...
    if mode == 'standard':
      ...
      copy_file(
          ...
          os.path.join(cefclient_win_dir, 'installer_config.json'),
          options.quiet)
      copy_file(
