...
#### C. Decide: Fix or Deprecate?

Before writing any code, determine whether the CEF API can still be correctly supported:

1. **Trace the full call chain.** Who calls this CEF method? What do callers expect? Check `cef/tests/` for test callers — tests reveal the expected contract (e.g., round-trip set/get verification).

2. **Verify correctness, not just compilation.** A fix that compiles but breaks caller expectations (e.g., a setter that no longer affects the getter) is worse than no fix. If there is no replacement Chromium API that preserves the same behavior, the CEF API should be deprecated.

3. **Deprecate when Chromium removes the underlying API.** If Chromium removed a public API that a CEF method wraps, and there is no equivalent replacement, deprecate the CEF method using the existing pattern:
    - **Public header** (`cef/include/`): Wrap in `#if CEF_API_REMOVED(CEF_NEXT)` and change `/*--cef()--*/` to `/*--cef(removed=next)--*/`
    - **Impl header** (`cef/libcef/`): Keep declarations (needed for `override`)
    - **Impl .cc**: Replace body with `CEF_API_REQUIRE_REMOVED(CEF_NEXT);`, comment out unused parameters as `/*param*/`
    - **Tests/samples**: Remove calls to the deprecated methods
    - **Auto-generated files** (`cef/libcef_dll/`): Do NOT edit — these are regenerated by the translator tool

4. **Search for existing patterns first.** Before inventing a solution, search the CEF codebase for how similar problems were already handled (e.g., `grep -r CEF_API_REMOVED cef/include/`).

5. **Complexity is a warning sign.** If your fix requires adding new helper methods, wrapper classes, or multi-layer abstractions, reconsider whether deprecation is the right approach.

For deprecating **enum values** rather than methods, see "Pattern 6: Enum Value Removed" below.

#### D. Apply the Fix (if not deprecating)
...
**Fix:** Remove references and mark CEF API as deprecated. For deprecating **methods** rather than enum values, see "Step C: Decide: Fix or Deprecate?" above.
```cpp
