...
1. Build your client application as a DLL (`client.dll`) instead of an executable. For windows applications (`/SUBSYSTEM:WINDOWS`) implement/export the `RunWinMain` function and run with `bootstrap.exe`. For console applications (`/SUBSYSTEM:CONSOLE`) implement/export the `RunConsoleMain` function and run with `bootstrapc.exe`. Function signatures are provided in [cef_sandbox_win.h](https://github.com/chromiumembedded/cef/blob/master/include/cef_sandbox_win.h#L98).
