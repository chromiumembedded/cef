...
IMPLEMENT_HAS_ONE_REF(simple_app_t, simple_app, ref_count)
IMPLEMENT_HAS_AT_LEAST_ONE_REF(simple_app_t, simple_app, ref_count)
...
IMPLEMENT_ADDREF(simple_browser_process_handler_t,
                 browser_process_handler,
                 ref_count)
IMPLEMENT_HAS_ONE_REF(simple_browser_process_handler_t,
                      browser_process_handler,
                      ref_count)
IMPLEMENT_HAS_AT_LEAST_ONE_REF(simple_browser_process_handler_t,
                               browser_process_handler,
                               ref_count)

// Release function for browser process handler.
int CEF_CALLBACK browser_process_handler_release(cef_base_ref_counted_t* self) {
  simple_browser_process_handler_t* handler =
      (simple_browser_process_handler_t*)self;
  int count = atomic_fetch_sub(&handler->ref_count, 1) - 1;
  if (count == 0) {
    // Don't release default_client - it's managed via the global instance.
    free(handler);
    return 1;
  }
  return 0;
...
simple_browser_process_handler_t* browser_process_handler_create(void) {
  ...
  handler->handler.base.add_ref = browser_process_handler_add_ref;
  handler->handler.base.release = browser_process_handler_release;
  handler->handler.base.has_one_ref = browser_process_handler_has_one_ref;
  handler->handler.base.has_at_least_one_ref =
      browser_process_handler_has_at_least_one_ref;
...
simple_app_t* simple_app_create(void) {
  ...
  app->app.base.add_ref = simple_app_add_ref;
  app->app.base.release = simple_app_release;
  app->app.base.has_one_ref = simple_app_has_one_ref;
  app->app.base.has_at_least_one_ref = simple_app_has_at_least_one_ref;
